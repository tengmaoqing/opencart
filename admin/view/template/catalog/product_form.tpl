...
<div id="content">
                        ...
                        <table id="option-value<?php echo $option_row; ?>" class="table table-striped table-bordered table-hover">
                          <thead>
                            <tr>
                              ...
                              <td class="text-right"><?php echo $entry_excludes; ?></td>
                              <td></td>
                            </tr>
                          </thead>
                          <tbody>
                            ...
                            <tr id="option-value-row<?php echo $option_value_row; ?>">
                              <td class="text-left"><select name="product_option[<?php echo $option_row; ?>][product_option_value][<?php echo $option_value_row; ?>][option_value_id]" class="form-control">
                                  ...
                                  <?php if ($option_value['option_value_id'] == $product_option_value['option_value_id']) { 
                                      $all_option_values[] = array(
                                        ...
                                        'label' => $product_option['name'].'/'.$option_value['name']
                                      )
                                    ?>
                              ...
                              <td class="text-left">
                                  <div data-toggle="modal">
                                    ...
                                    <input type="hidden" name="product_option[<?php echo $option_row; ?>][product_option_value][<?php echo $option_value_row; ?>][option_value_excludes]" value="<?php echo $product_option_value['option_value_excludes'] ?>">
                                  </div>
  ...
  <div id="optionsModal" class="modal fade bs-example-modal-sm" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel">
    <div class="modal-dialog modal-sm" role="document">
      <div class="modal-content">
        <form>
          <?php
          foreach ($all_option_values as $opt) : ?>
          <label style="margin-right: 10px;">
            <input type="checkbox" value="<?php echo($opt['value']) ?>"> <?php echo($opt['label']) ?>
          </label>
          <?php endforeach ?>
        </form>                  
      </div>
...
<script>
  ;(function() {
    var allOptions = '<?php echo json_encode($all_option_values, 1); ?>';
    var optionsModal = $('#optionsModal');
    var resetForm = () => {
      optionsModal.find('form').get(0).reset();
    };
    var initForm = (data) => {
      resetForm();
      optionsModal.find('form input').each((i, input) => {
    ...
    var currentExInput = null;
    $('#content [data-toggle="modal"]').on('click', function() {
      optionsModal.modal('show');
      currentExInput = $(this).find('input');
      initForm(currentExInput.val().split(','));
    });

    optionsModal.on('hidden.bs.modal', function() {
      var data = getFormData();
      currentExInput.val(data);
    });
